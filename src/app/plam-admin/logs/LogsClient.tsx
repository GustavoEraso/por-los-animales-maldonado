...
/**
 * Parses a 'YYYY-MM-DD' date input into the local start/end of that day.
 *
 * @param value - Date input string in 'YYYY-MM-DD' format
 * @param endOfDay - If true, returns the last millisecond of the day
 * @returns A Date object for the requested boundary, or null if the input is invalid
 */
...
export default function LogsClient() {
  ...
  async function handleFetch() {
    ...
    try {
      ...
      // systemAuditLog.date is a number (ms); logs.createdAt is a Firestore Timestamp
      const isTimestampField = dateField === 'createdAt';

      if (dateFrom) {
        const startBoundary = parseDateBoundary(dateFrom, false);
        if (startBoundary) {
          constraints.push(
            where(dateField, '>=', isTimestampField ? startBoundary : startBoundary.getTime())
          );
        }
      }
      if (dateTo) {
        const endBoundary = parseDateBoundary(dateTo, true);
        if (endBoundary) {
          constraints.push(
            where(dateField, '<=', isTimestampField ? endBoundary : endBoundary.getTime())
          );
        }
      }
      ...
      setPage(0);
      setFetched(true);
      setReachedLimit(results.length >= MAX_RESULTS);
    } catch (err) {
      logger({
        level: 'error',
      ...
      <div className="flex flex-col gap-4 sm:gap-6 p-3 sm:p-6 w-full max-w-7xl">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 bg-white rounded-xl p-3 sm:p-6 shadow-sm">
            ...
            <>
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  value={modifiedBySearch}
                  onChange={(e) => setModifiedBySearch(e.target.value)}
                  className={inputClass}
                />
              </div>
            </>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 items-end sm:col-span-2 lg:col-span-3">
            ...
            <button
              onClick={handleClear}
              className="rounded-full border border-gray-300 px-4 sm:px-6 py-2 text-xs sm:text-sm font-semibold text-gray-700 transition-all duration-300 hover:bg-gray-100"
            >
              Limpiar filtros
            </button>
            {fetched && (
            ...
            {fetched && reachedLimit && (
              <span className="text-xs sm:text-sm text-amber-600 self-center ml-auto">
                Mostrando los primeros {MAX_RESULTS} resultados — afiná el rango de fechas para ver
                más
              </span>
            )}
          </div>
          ...
          <>
            {logs.length === 0 ? (
              <div className="bg-white rounded-xl p-6 sm:p-12 text-center text-gray-500">
                <EyeIcon
                  size={36}
                  className="sm:size-48 mx-auto mb-4 opacity-30"
                  ...
                  <table className="w-full text-xs sm:text-sm">
                    <thead>
                      <tr className="border-b border-gray-200 bg-gray-50">
                        <th className="text-left px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-600">
                          Fecha
                        </th>
                        <th className="text-left px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-600">
                          Tipo
                        ...
                        <th className="hidden sm:table-cell text-left px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-600">
                          Modificado por
                        </th>
                        <th className="hidden md:table-cell text-left px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-600">
                          ID
                        </th>
                        <th className="text-left px-2 sm:px-4 py-2 sm:py-3 font-semibold text-gray-600">
                          Cambios
                        ...
                        return (
                          ...
                          >
                            <td className="px-2 sm:px-4 py-2 sm:py-3 whitespace-nowrap text-gray-700">
                              {formatDate(d._date as number)}
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                              <span className="text-gray-700">
                                {getTypeLabel(d.type as string)}
                              </span>
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                              <span
                                className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${getActionBadge(d.action as string)}`}
                              >
                                {getActionLabel(d.action as string)}
                              </span>
                            </td>
                            <td
                              className="px-2 sm:px-4 py-2 sm:py-3 text-gray-700 max-w-[120px] sm:max-w-[200px] truncate"
                              title={(d.entityName as string) ?? (d.entityId as string)}
                            >
                              {(d.entityName as string) ?? '—'}
                            </td>
                            <td
                              className="hidden sm:table-cell px-2 sm:px-4 py-2 sm:py-3 text-gray-700 max-w-[150px] truncate"
                              title={(d.modifiedByName as string) ?? (d.modifiedBy as string)}
                            >
                              {(d.modifiedByName as string) ?? (d.modifiedBy as string)}
                            </td>
                            <td className="hidden md:table-cell px-2 sm:px-4 py-2 sm:py-3">
                              <code className="text-xs text-gray-500">
                                {(d.entityId as string)?.slice(0, 8)}...
                              </code>
                            </td>
                            <td className="px-2 sm:px-4 py-2 sm:py-3">
                              {d.changes ? (
                                <button
                                  onClick={() =>
                                    setSelectedData(d.changes as Record<string, unknown>)
                                  }
                                  className="text-xs font-semibold text-amber-sunset hover:text-caramel-deep underline transition-colors whitespace-nowrap"
                                >
                                  Ver
                                </button>
                              ) : (
                      ...
                      {logs.slice(page * ITEMS_PER_PAGE, (page + 1) * ITEMS_PER_PAGE).map((log) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-2 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                              <span
                                className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${getLevelBadge(d.level as string)}`}
                              >
                            ...
                            <td
                              className="px-2 sm:px-4 py-2 sm:py-3 text-gray-700 max-w-[200px] sm:max-w-[300px] truncate"
                              title={d.message as string}
                            >
                              {d.message as string}
                            </td>
                            <td className="hidden sm:table-cell px-2 sm:px-4 py-2 sm:py-3 text-gray-500">
                              {(d.errorType as string) ?? '—'}
                            </td>
                            <td className="hidden md:table-cell px-2 sm:px-4 py-2 sm:py-3">
                              {d.statusCode != null ? (
                                <code className="text-xs text-gray-500">
                                  {d.statusCode as number}
                                </code>
                              ) : (
                                '—'
                              )}
                            ...
                            <td className="px-2 sm:px-4 py-2 sm:py-3">
                              {hasData ? (
                                <button
                                  onClick={() => setSelectedData({ data: d.data })}
                                  className="text-xs font-semibold text-amber-sunset hover:text-caramel-deep underline transition-colors whitespace-nowrap"
                                >
                                  Ver
                                </button>
        ...
        {!fetched && !loading && !showLoading && (
          <div className="bg-white rounded-xl p-6 sm:p-12 text-center text-gray-500">
            ...
            <p className="text-base sm:text-lg font-semibold">
              Usá los filtros y presioná &quot;Buscar logs&quot;
            </p>
            <p className="text-xs sm:text-sm">
              Mostrará hasta {MAX_RESULTS} resultados ordenados por fecha descendente, en páginas de{' '}
              {ITEMS_PER_PAGE}.
            </p>
          </div>
        )}
