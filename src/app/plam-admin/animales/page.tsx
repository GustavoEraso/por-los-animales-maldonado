        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50">
            <tr>
              ...
              <th className="w-fit hidden sm:table-cell" scope="col" >
                <button className=" py-3 px-2 md:px-4 lg:px-6 w-full text-start hover:bg-gray-300" onClick={() => sortAnimalBy({ reference: 'isAvalible' })}>{`VISIBLE ${renderDirection('isAvalible')}`}</button>
              </th>
              ...
              animalsToShow.length > 0 && sortedAnimals?.map((animal) => (
                ...
                <tr key={animal.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-2 py-4 text-right  hidden sm:table-cell">
                    <Link href={`/plam-admin/animales/editar/${animal.id}`} className="font-medium text-blue-600  hover:underline">Editar</Link>
                  </td>
                  <td className="px-2 py-4 text-right hidden sm:table-cell">
