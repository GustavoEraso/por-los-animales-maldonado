...
export default function ConfirmarMatchesContent(): React.ReactElement {
  ...
  const handleConfirmCreateAnimal = async (): Promise<void> => {
    ...
    if (!f.name.trim()) {
      ...
      handleToast({ type: 'error', title: 'Falta el nombre', text: 'El nombre del animal es obligatorio.' });
      return;
    }

    setCreateFormSaving(true);
    try {
      const id = await generateAnimalId(f.name.trim());
      ...
      const confirmedEntry: ConfirmedEntry = {
        ...
        migratedTo: id,
        migratedFields: [
          'contactName', 'contacts', 'address', 'followUpManager',
          'newName', 'isSterilized', 'sterilizationDate',
          'notas_seguimiento', 'notas_plam',
        ].filter((field) => {
      ...
      const newCurrentEntries = matchesData.filter(
        (e) => e.csv_row !== createFormEntry.csv_row
      );
  ...
  const handleMigrate = async (): Promise<void> => {
    ...
    const existingNotes: string[] = Array.isArray(rawNotes) ? rawNotes : rawNotes ? [String(rawNotes)] : [];
    ...
    try {
      ...
      const confirmedEntry: ConfirmedEntry = {
        ...
        animalImageUrl:
          ...
          selectedPrivateInfo?.mainImageUrl ??
          '',
      };

      // Prepare new arrays for optimistic update
      const newCurrentEntries = matchesData.filter(
        (e) => e.csv_row !== selectedEntry.csv_row
      );
      const newConfirmedEntries = [...confirmedData, confirmedEntry];
  ...
  const comparisonFields = useMemo((): MigrateField[] => {
    ...
    return [
      ...
      },
      {
        key: 'isSterilized',
        label: 'Castrado',
        currentValue: pi?.isSterilized === 'si' ? 'Sí' : pi?.isSterilized === 'no' ? 'No' : 'No se sabe',
        ...
        <div className="w-full max-w-7xl">
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((entry) => {
                const d = entry.csv_data;
                const isConfirmed = 'migratedAt' in entry && (entry as ConfirmedEntry).migratedAt > 0;
                const isMatched =
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-xl shadow-sm border-2 p-4 flex flex-col gap-3 ${
                      isConfirmed ? 'border-blue-300' : isMatched ? 'border-green-300' : 'border-red-200'
                    }`}
                  >
                    {/* Match status badge */}
                    <div className="flex items-center justify-between">
                      ...
                      {isConfirmed && (
                        <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                          {new Date((entry as ConfirmedEntry).migratedAt).toLocaleDateString('es-UY', {
                            day: '2-digit',
                            month: '2-digit',
                          })}
                        </span>
                      ...
                      <div className="text-xs bg-blue-50 text-blue-800 p-2 rounded">
                        ...
                        {entry.all_matched_ids.length > 1 && (
                          <p className="truncate">
                            IDs:{' '}
                            {entry.all_matched_ids.map((id) => getAnimalNameFromId(id)).join(', ')}
                          </p>
                        )}
            ...
            <section className="flex flex-col items-center justify-start bg-cream-light w-full p-6 gap-4 text-left pb-28 min-h-full">
                ...
                <>
                  ...
                  <div className="w-full max-w-2xl">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Abre un formulario pre-rellenado con los datos del CSV para revisar y confirmar
                    </p>
                  </div>
              ...
              {selectedAnimal && selectedEntry && (
                <>
                  ...
                  {/* Selected count */}
                  <p className="text-xs text-gray-500">
                    {fieldsToMigrate.size} campo{fieldsToMigrate.size !== 1 ? 's' : ''} seleccionado
            ...
            <section className="flex flex-col items-center justify-start bg-cream-light w-full p-6 gap-4 text-left overflow-hidden max-h-full">
              ...
              <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100 shrink-0">
                {([
                  ...
                  <div
                    key={label}
                    className="grid grid-cols-[140px_1fr] gap-3 p-3 text-sm"
                  >
                    <span className="font-semibold text-green-dark shrink-0">{label}</span>
                    <span className={`break-all ${value ? 'text-gray-700' : 'text-gray-400 italic'}`}>
                      {value || '(vacío)'}
                    </span>
                  </div>
                ))}
          ...
          >
            <section className="flex flex-col items-center justify-start bg-cream-light w-full p-6 gap-4 text-left overflow-hidden max-h-full">
              ...
              <p className="text-sm text-gray-500 text-center -mt-2">
                Revisá los datos antes de crear. Los campos de info privada se guardan automáticamente.
              </p>
              ...
              <div className="w-full max-w-lg flex flex-col gap-4 overflow-y-auto flex-1 min-h-0 pr-1">
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setCreateFormField('species', e.target.value as 'perro' | 'gato' | 'otros')}
                  >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setCreateFormField('gender', e.target.value as 'macho' | 'hembra')}
                  >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) => setCreateFormField('castrado', e.target.value as 'si' | 'no' | 'no_se')}
                  >
                ...
                {/* Responsable */}
                <label className="flex flex-col gap-1">
                  <span className="font-semibold text-green-dark text-sm">Responsable seguimiento</span>
                  <input
                    type="text"
                    className="outline-2 bg-white outline-gray-200 rounded p-2 text-sm"
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
                  El animal se creará como <strong>adoptado</strong> y <strong>no visible</strong> en la web.
                </div>
              </div>
