 ...
 * Allows selecting sponsors, setting direction and speed.
 */
export default function CreateCarouselPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setSponsors(res.sort((a, b) => a.name.localeCompare(b.name)));
    };
    load();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const id = `carousel-${Date.now().toString(36)}`;
    ...
    try {
      await handlePromiseToast(
        ...
        {
          messages: {
            pending: { title: 'Guardando carrusel...', text: 'Por favor espera...' },
            success: { title: 'Carrusel creado', text: 'El carrusel ya está disponible' },
  ...
  return (
    <ProtectedRoute requiredRole="admin" redirectPath="/plam-admin">
      <main className="flex flex-col gap-6 p-4 md:px-16 pb-24 max-w-3xl mx-auto">
        <h1 className="text-3xl font-bold">Nuevo Carrusel</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="flex-1 py-3 bg-green-forest text-white rounded-lg hover:bg-green-dark transition-colors font-semibold"
            >
              Guardar carrusel
            </button>
            <button
