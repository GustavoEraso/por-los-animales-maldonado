...
interface CompartirClientProps {
  ...
  initialContacts: WpContactType[];
  initialLitterAnimals: Animal[];
}

/**
 * Client-side share-image editor.
 *
 * @param props - Initial data loaded by the server wrapper
 * @returns The interactive share-image editor
 */
export default function CompartirClient({
  initialAnimal,
  initialContacts,
  initialLitterAnimals,
}: CompartirClientProps): React.ReactElement {
  return (
    <CompartirContain
      initialAnimal={initialAnimal}
      initialContacts={initialContacts}
      initialLitterAnimals={initialLitterAnimals}
    />
  );
}
...
function CompartirContain({
  ...
  const selectedContacts =
    initialContacts.length >= 2 ? [0, 1] : initialContacts.length === 1 ? [0] : [];
  const litterAnimals = initialLitterAnimals;
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setLoading(false), MIN_LOADING_TIME);
  ...
  const capturar = async () => {
    ...
    try {
      ...
      logger({
        level: 'error',
        code: 'CAPTURE_IMAGE_ERROR',
        message: error instanceof Error ? error.message : 'Error capturing image',
        data: error,
      });
      handleToast({
        type: 'error',
